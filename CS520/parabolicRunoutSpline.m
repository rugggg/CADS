...
cubicM = zeros(length(x)-2,1);

for i = 1:length(cubicM)
  cubicM(i) = (y(i)-(2*y(i+1))+y(i+2));
end
...
Mtemp = inv(A)*B; % CHANGE the name of Mtemp as is needed for the correct runout
M = zeros(length(x),1);
for i = 1:length(x)
  if(i == 1 || i == 2)
    M(i) = Mtemp(1);
...
title('Parabolic Runout Spline') % CHANGE the name lol
xlabel('Temp')
